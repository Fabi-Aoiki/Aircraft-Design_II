...
Re = v_cruise * l_f /isa.isa_model(0, con.dt)[5]
...
Re = v_cruise * l_n / isa.isa_model(0,con.dt)[5]
C_Ftu = 0.455 / (math.log10(Re))**2.58 # friction coefficient turbulent flow
...
cd_ground_list = ExtrParamTxt("Aircraft-Design_II/CL_CD_Ground.txt", "CD")
cl_ground_list = ExtrParamTxt("Aircraft-Design_II/CL_CD_Ground.txt", "CL")

cd_nac_list = []
cl_nac_list = cl_ground_list
for i in range(len(cd_ground_list)):
    cd_nac_list.append(cd_ground_list[i] + c_Dn + c_Df)

plt.plot(cd_nac_list, cl_nac_list, linestyle = "-", label = "Complete Drag")


# export txt

max_len = max(len(cl_nac_list), len(cd_nac_list))

# Export to TXT with columns
with open('CL_CD_Ground_Result.txt', 'w') as f:
    f.write('CL\tCD\n')
    for i in range(max_len):
        a = cl_nac_list[i] if i < len(cl_nac_list) else ''
        b = cd_nac_list[i] if i < len(cd_nac_list) else ''
        f.write(f"{a}\t{b}\n")
